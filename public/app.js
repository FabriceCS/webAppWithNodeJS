...
// === Functions ==
// -- API --
const fetchAllWhispers = () => fetch(`/api/v1/whisper`).then((response) => response.json())
const deleteWhisper = (id) => fetch(`/api/v1/whisper/${id}`, { method: 'DELETE' })
const updateWhisper = (id, message) => fetch(`/api/v1/whisper/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message }) })
const createWhisper = (message) => fetch(`/api/v1/whisper`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message }) })
